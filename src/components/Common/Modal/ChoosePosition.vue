<template>
	<div style="font-size:14px;">
		...
		<div class="modal fade" :id="personModal" tabindex="-1" role="dialog" aria-labelledby="personModalLabel">
			<div class="modal-dialog modal-lg" role="document" style="width:90%;height:34%;">
				<div class="modal-content" style="width:100%;height:100%;overflow-y:auto;">
					<div class="modal-header">
						<button type="button" class="close" data-dismiss="modal" aria-label="Close" @click="closemodal()"><span aria-hidden="true">&times;</span></button>
						<h4 class="modal-title" id="personModalLabel">Choose Position</h4>
					</div>
														...
														<form role="form-horizontal" style="background-color: #F1F5F8">
															<div class="box-body">
																<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
																	...
																	<div>
																		<input type="text" :readonly="inputReadonly" :id="'division'+personModal" class="form-control col-sm-4 Staffcode" style="width: 100%" placeholder="Division">
																	</div>
																</div>
																<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
																	<label class="control-label pad-top-7">
																	...
																	<div>
																		<input type="text" :readonly="inputReadonly" class="form-control" :id="'department'+personModal" style="width: 100%" placeholder="Department">
																	</div>
																</div>
																<div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
																	<label class="control-label pad-top-7">
	...
	export default {
		...
		methods: {
			popmodal: function() {
				...
				setTimeout(function() {
					$('#selectotherperson').attr("class", "");//input-group-btn会影响表格
				}, 300);
			...
			retrieveData: function(url, aoData, fnCallback) {
				...
				var roleType=this.form.roleType;
				if(roleType=="AmMkOmk"){
					$("#division"+this.personModal).val(this.form.creatorDivision);
					$("#department"+this.personModal).val(this.form.creatorDepartment);
				}
				//if(roleType=="Orga Partner"){
				//	var division="";
				//	var department="";
				//}else{
				//	var division=this.form.creatorDivision;
				//	var department=this.form.creatorDepartment;
				//}
				var division=$("#division"+this.personModal).val();
				var department=$("#department"+this.personModal).val();
				...
				console.log("positionCode="+positionCode+",positionName="+positionName);
				console.log("division="+division+",department="+department);
				$.ajax({
