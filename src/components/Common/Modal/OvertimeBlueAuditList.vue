<template>
	<div style="font-size:14px;">
		...
		<div class="modal fade" id="OTBAuditModal" tabindex="-1" role="dialog" aria-labelledby="submitModalLabel">
			<div class="modal-dialog modal-sm" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<span id="submitNoticecontent">
						Please be noted that there {{this.headcountnumber==1?'is':'are'}} {{this.headcountnumber}} {{this.headcountnumber==1?'employee’s':'employees’s'}} planned additional working plan hours {{this.headcountnumber==1?'has':'have'}} exceeded 20 hours. You can click <a @click="popmodal();" title="List">List</a> button to see more details.请注意有{{this.headcountnumber}}位员工本月预额外工作计划时数超过20小时。您可点击<a @click="popmodal();" >列表</a>按钮查看更多详细信息。</span>
					</div>
		...
		<div class="modal fade" :id="personModal" tabindex="-1" role="dialog" aria-labelledby="personModalLabel">
			<div class="modal-dialog modal-lg" role="document" style="width:80%;height:70%;">
														...
														<table :id="person_table" class="table table-bordered dataTable" role="grid" aria-describedby="example2_info" style="margin-top: 20px;">
															<thead>
																<tr role="row" style="height:20px;width:100%;">
																	...
																	<th>Monthly Accumulated Applied Hours <br>月平均时数<i class="fa fa-fw fa-info-circle text-info" title="Comprehensive working hours calculation cycle 综合工时计算周期内"></i></th>
...
<script>
	export default {
		name: "OvertimeBlueAuditList",
		props: ['chooseID','overtimeIds','overtimePeriod', 'headcountnumber'],
		methods: {
			popmodal: function() {
			...
			retrieveData: function(url, aoData, fnCallback) {
				...
				if(overtimeIds.length > 0) {
					for(var i = 0; i < overtimeIds.length; i++) {
						overtimeIdstrs += overtimeIds[i] + ",";
					}
					overtimeIdstrs = overtimeIdstrs.substring(0, overtimeIdstrs.length-1);
				}
