...
<script>
	export default {
		name: "ChooseBlueCollar",
		props: ['vclass','chooseID','isForeman','creatorStaffcode','direct','detailIndex'],
		methods: {
			popmodal: function() {
			...
			retrieveData: function(url, aoData, fnCallback) {
				...
				var creatorStaffcode = this.creatorStaffcode;
				//console.log('direct='+direct);
				//console.log('creatorStaffcode='+creatorStaffcode);
				$.ajax({
		...
		mounted() {
			var path = "/overTimeBlue/listUserInfoByDirect";
			if(this.isForeman) {
				path = "/overTimeBlue/listUserInfoByForeman";
			}
			...
			var table = $("#"+this.person_table).DataTable({
				...
				"sAjaxSource": Config.getBaseURL() + path,
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
