<template>
	<div class="input-group date bootstrap-timepicker" style="width:100%;">
		...
		<input :id="'timepicker-' + currTimes" v-model="selectValue" type="text" class="form-control pull-right timepicker input" :name="realname" :class="{'is-danger': errors.has(realname) }" :placeholder="placeholder" v-validate="rules">
	</div>
	<!-- /.input group -->
</template>
<script>
	export default {
		name: "Timepicker",
		props: ["value", "placeholder", "name", "otherRules"],
		inject: {
			$validator: '$validator'
		},
		methods: {
