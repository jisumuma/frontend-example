...
<script>
	export default {
		name: "Datepicker",
		props: ["value", "placeholder", "dateformate", "startDate","endDate", "maxViewMode", "minViewMode", "name", "otherRules"],
		inject: {
			$validator: '$validator'
		},
		methods: {
		...
		mounted() {
			...
			var startDate = this.selectStartDate;
			var endDate = this.selectEndDate;
			var _maxViewMode = "4";
			var _minViewMode = "0";
			if(this.maxViewMode) {
				_maxViewMode = this.maxViewMode
			}
			if(this.minViewMode) {
				_minViewMode = this.minViewMode
			}
			if(this.dateformate) {
				...
				$('#datepicker-' + this.currTimes).datepicker("setDate", format(this.value));
		        $('#datepicker-' + this.currTimes).on("changeDate", function(e) {
		          if(!$(this).val()){
		            return;
		          }
