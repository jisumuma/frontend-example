    ...
    <!-- <div v-for="(entry,index) in attachments" :id="'file-id-'+entry.id" :data-attachment-id="entry.id" :x-file-id="id">
			...
			<div :id="'inline-file-'+id" style="display:block;margin-top:5px;" class="users-list clearfix upload-images-list">
				<li v-for="(entry,index) in attachments" :id="'file-id-'+entry.id" :data-attachment-id="entry.id" :x-file-id="id">
					<div><img style="height: 80px;width: 100px;" :src="GL.getURL('/attachmentlob/downloadimage/'+entry.id)" :alt="entry.attachName"></div>
					<a class="users-list-name" target='_blank' :href="GL.getURL('/attachmentlob/download/'+entry.id)"><i class="fa fa-download"></i>&nbsp;Download 下载</a>
...
export default {
  ...
  mounted() {
    this.$root.UPIMAGELOB.File.init(this.id);
    var sid = "upload-" + this.id;
    var change = this.changed;
    var remove = this.$root.UPIMAGELOB.File.onRemove;
    $(function() {
