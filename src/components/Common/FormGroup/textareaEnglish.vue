<template>
  <div :id="divid">
    ...
    <p>{{limitSize}}{{$t("textareaNotice3")}}</p>
  </div>
</template>

<script>
  export default {
    name: 'textareaEnglish',
    props:["name","value","readonly","placeholder","otherRules","maxlength","disabled","id","rows","limit","hasstyle"],
    inject: {
      $validator: '$validator'
    },
    data(){
      return {
        ...
        limitSize:this.limit?this.limit:100
      }
    },
    watch: {
      "value":{
      ...
      $("div#"+_this.divid).on('blur','textarea.form-control',function() {
        ...
        if($(this).attr("readonly")) {
          ...
          if($(this).val()) {
            ...
            $.alertApon(_this.limitSize+' Characters Max.');
          }
        }
      });
    },
