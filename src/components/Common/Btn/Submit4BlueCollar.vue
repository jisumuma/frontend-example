<template>
	<div>
		<button type="button" id="submitBlue" @click="submit()" :class="'btn btn-success pull-left ' + vclass"><i class="fa fa-fw fa-check"></i>&nbsp;{{reallabelName}}</button>
		<!-- submitModal start-->
			...
			<div class="modal-dialog modal-sm" role="document">
				<div class="modal-content">
					<div class="modal-body"><!-- when previous application status is finished -->
						...
						<span>请注意该员工在该时段已有额外工作计划申请，您确定要覆盖它吗？覆盖功能会导致该员工上一笔额外工作计划记录作废，但其他员工的记录不受影响。您可点击<a @click="popmodal();" title="List">列表</a>按钮查看作废的详情记录。</span>
					</div>
					<div class="modal-footer">
		...
		methods: {
			...
			confirm: function() {
				...
				for(var i=0;i<this.form.overtimeDetails.length;i++){
					var overlapId = this.form.overtimeDetails[i].addfirst;
					if(overlapId) {
						this.form.existCompleteCnt++;
						if(overlapId.indexOf(',') < 0) {
							this.form.existCompleteIds.push(overlapId);
						} else {
							var ids = overlapId.split(',');
							for(var j=0;j<ids.length;j++){
								this.form.existCompleteIds.push(ids[i]);
							}
						}
					}
				}
			...
			startProcess: function() {
				...
				for(var i=0;i<this.form.overtimeDetails.length;i++){
					this.form.overtimeDetails[i].status = 1;
				}
