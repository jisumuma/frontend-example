        ...
        <div class="col-md-12">
            ...
            <div class="box box-primary1">
                <div class="box-header">
                    <h3 class="box-title">
                        Construction Work Permit CE Permit Responsible Tasks
                    </h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                                ...
                                <form role="form-horizontal" style="background-color: #F1F5F8">
                                    <div class="box-body">
                                        <div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
                                            ...
                                            <div>
                                                <input type="text" id="task-business-key" class="form-control col-sm-4" style="width: 100%" placeholder="Ticket No. 申请单号">
                                            </div>
                                        </div>
                                        <!--
                                        <div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
                                        ...
                                        <div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
                                            ...
                                            <div>
                                                <input type="text" class="form-control" id="task-applicant" style="width: 100%" placeholder="Applicant 申请人">
                                            </div>
                                        </div>
                                        <div class="form-group col-xs-12 col-sm-12 col-md-6 col-lg-3">
                                            <label class="control-label pad-top-7">
                                            ...
                                            <div>
                                                <input type="text" class="form-control" id="task-staffcode" style="width: 100%" placeholder="Staff Code 员工号">
                                            </div>
                                        </div>
	...
	export default{
		...
		methods:{
	    	...
	    	retrieveData:function(url,aoData, fnCallback){
                ...
                var taskStaffcode = $("#task-staffcode").val();
                var taskCreatedBefore = "";
                var taskCreatedAfter = "";
	    		...
	    		$.ajax({
			        ...
			        data : {"aoData":JSON.stringify(aoData),"applicantId":applicantId,"taskCreatedBefore":taskCreatedBefore,"taskCreatedAfter":taskCreatedAfter,"processDefinitionKey":processDefinitionKey,"processBusinessKey":processBusinessKey,"staffcode":taskStaffcode},//这个是把datatable的一些基本数据传给后台,比如起始位置,每页显示的行数
			        type : 'get',
			        dataType : 'json',
		...
		mounted(){
			...
			var table = $("#task_table").DataTable({
                ...
                //"sAjaxSource": Config.getBaseURL()+"/base/tasks/listCePermit?userId="+applicantId,
                "sAjaxSource": Config.getBaseURL()+"/constructionWorkPermit/listCePermit?userId="+applicantId,
				"serverSide": true,
				"filter":false,
				"lengthChange": false,
                ...
                "fnRowCallback" : function(row, data,index) {//设置列从0开始
                    ...
                    $('td:eq(1)', row).html(data.department);
                    $('td:eq(2)', row).html(data.staffcode);
                    $('td:eq(3)', row).html(data.applicantName);
					//$('td:eq(3)', row).html(format(data.applicantTime)); veeFnt()
					$('td:eq(4)', row).html(data.arrivetime);
					//$('td:eq(5)', row).html( '<span class="label label-success">'+data.applicantName+'</span>');
					//$('td:eq(5)', row).html('<a href="javascript:;" style="margin-left: 10px;"> <i class="fa fa-search task-detail"></i></a>');
					return row;
				}
			});
            ...
            var myTable = $('#task_table').DataTable();
            $('#task_table').on('click', 'tr',function() {
                var data = myTable.row(this).data(); //获取单击那一行的数据
                //console.log(data);
                //alert( 'You clicked on  row' );
                console.log("processInstanceKey="+data['processInstanceKey']+",businessKey="+data['businessKey']+",taskId="+data['taskId']+",taskKey="+data['taskKey']+",processInstanceId="+data['processInstanceId']);
                if(data['status'] == 2){
                    router.push({ path: '/index/form/'+data['processInstanceKey']+'/create?id='+data['businessKey']});
                }else if(data['status'] == 9 ){
                    router.push({ path: '/index/form/'+data['processInstanceKey']+'/reAudit/'+data['businessKey']+'?taskId='+data['taskId']+'&taskKey='+data['taskKey']+'&processInstanceId='+data['processInstanceId']+'&businessKey='+data['businessKey']});
