				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Blue Collar Planned Additional Working Time Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="OverTimeBlueReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "OverTimeBlueReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#OverTimeBlueReport").DataTable().draw();
			},
			otbs: function() {
				var url = Config.getBaseURL() + "/overTimeBlue/batchCreateOTBS";
				$.ajax({
					...
					success: function(data) {
						$.alert("Batch create OTBS application successfully.");//console.log(JSON.parse(data));
					},
					error: function(msg) {
						$.alert("There is no OTBS application.");
					}
				});
			},
			otbslm: function() {
				var url = Config.getBaseURL() + "/overTimeBlue/batchCreateLastMonthOTBS";
				$.ajax({
					url: url, //这个就是请求地址对应sAjaxSource
					data: {},
					type: 'post',
			...
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/overTimeBlue/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#OverTimeBlueReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/overTimeBlue/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					$('td:eq(3)', row).html('Blue Collar Planned Additional Working Time');
					$('td:eq(4)', row).html('-');
			...
			$('#OverTimeBlueReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/OverTimeBlue/print?id=" + data.id + "&token=" + Config.getToken();
