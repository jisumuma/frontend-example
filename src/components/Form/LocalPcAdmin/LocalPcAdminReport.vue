				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;PC Local Administrator Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="LocalPcAdminReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "LocalPcAdminReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#LocalPcAdminReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/localPcAdmin/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#LocalPcAdminReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/localPcAdmin/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('PC Local Administrator');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#LocalPcAdminReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/LocalPcAdmin/print?id=" + data.id + "&token=" + Config.getToken();
