				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Untaken Annual Leave Carry-over Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<form>
								<div class="text-muted well well-sm no-shadow">
									...
									<!--<div class="row">
										<div class="form-group col-md-3" style="padding-top: 24px;margin-bottom: 0;">
											<button type="button" class="btn btn-primary mar-left-5" @click="pushdata();" title="Push Data" style="margin-left: 5px;"><i class="fa fa-fw fa-external-link"></i> Push Data</button>
										</div>
									</div>-->
								</div>
							</form>
							<table id="HrUalCarryoverReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "HrUalCarryoverReport",
		props: ['vclass'],
		methods: {
			pushdata:function(){
				var uri = "/hrualcarryover/pushdatatohrone" ;
			...
			draw: function() {
				$("#HrUalCarryoverReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/hrualcarryover/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#HrUalCarryoverReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/hrualcarryover/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Untaken Annual Leave Carry-over');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#HrUalCarryoverReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/HrUalCarryover/print?id=" + data.id + "&token=" + Config.getToken();
