				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Overtime Plan Sheet Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<form>
								<div class="text-muted well well-sm no-shadow">
									...
									<div class="row">
										<!-- 申请单号 -->
										<div class="form-group col-md-3">
											<label for="">Process Type 流程类别</label>
											<select class="form-control" id="type">
												<option value="">-- Please select--</option>
												<option value="OTS">Overtime Plan Sheet</option>
												<option value="OTSS">Overtime Plan Sheets</option>
											</select>
										</div>
										<!-- 搜索、导出按钮 -->
							...
							<table id="OverTimeWhiteSheetReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "OverTimeWhiteSheetReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#OverTimeWhiteSheetReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/overTimeWhite/exportrexcelsheet?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#OverTimeWhiteSheetReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/overTimeWhite/reportsheet",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					$('td:eq(1)', row).html(data.approver);
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html((data.processDefinitionKey == 'OverTimeWhiteSheet') ? 'Overtime Plan Sheet' : 'Overtime Plan Sheets');
					$('td:eq(4)', row).html('-');
			...
			$('#OverTimeWhiteSheetReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/" + data.processDefinitionKey + "/print?id=" + data.id + "&token=" + Config.getToken();
