				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;IT System Account Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="ItSystemAccountReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "ItSystemAccountReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#ItSystemAccountReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/ItSystemAccount/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#ItSystemAccountReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/ItSystemAccount/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('IT System Account');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#ItSystemAccountReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/ItSystemAccount/print?id=" + data.id + "&token=" + Config.getToken();
