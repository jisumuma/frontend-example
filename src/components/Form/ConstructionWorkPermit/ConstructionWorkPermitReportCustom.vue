				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Construction Work Permit Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="ConstructionWorkPermitReportCustom" class="table table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "ConstructionWorkPermitReportCustom",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#ConstructionWorkPermitReportCustom").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/constructionWorkPermit/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#ConstructionWorkPermitReportCustom").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/constructionWorkPermit/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					var sub='';
					if(data.permitCategory=='Construction Permit'){
						sub='Construction Permit 施工许可';
					}else if(data.permitCategory=='Hot Work Permit'){
						sub='Hot Work Permit 动火作业许可证';
					}else if(data.permitCategory=='Energy & Medium Application'){
						sub='Energy & Medium Application 能源 & 介质申请';
					}else if(data.permitCategory=='Work at Height Permit'){
						sub='Work at Height Permit 高处作业许可证';
					}else if(data.permitCategory=='Confine Space Permit'){
						sub='Confine Space Permit 有限空间许可证';
					}else if(data.permitCategory=='Other Dangerous Work'){
						sub='Other Dangerous Work 其他危险作业';
					}else{
						sub = data.permitCategory;
					}
					
					$('td:eq(0)', row).html(data.ticketNo);
					$('td:eq(1)', row).html(data.creatorName);
					$('td:eq(2)', row).html(data.createUserName != data.creatorName ? 'By ' + data.createUserName : '-');
					$('td:eq(3)', row).html('Construction Work Permit');
					$('td:eq(4)', row).html(sub);
			...
			$('#ConstructionWorkPermitReportCustom tbody').on('click', '.exportpdf', function() {
