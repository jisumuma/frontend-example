				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;KVS Account Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="KvsAccessGeneratorReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "KvsAccessGeneratorReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#KvsAccessGeneratorReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/KvsAccessGenerator/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#KvsAccessGeneratorReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/KvsAccessGenerator/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
          ...
          if(data.status == 0) {
			...
			$('#KvsAccessGeneratorReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/KvsAccessGenerator/print?id=" + data.id + "&token=" + Config.getToken();
