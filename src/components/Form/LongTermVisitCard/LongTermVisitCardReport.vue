				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Long Term Visit Card Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="LongTermVisitCardReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "LongTermVisitCardReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#LongTermVisitCardReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/LongTermVisitCard/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#LongTermVisitCardReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/LongTermVisitCard/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Long Term Visit Card');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#LongTermVisitCardReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/LongTermVisitCard/print?id=" + data.id + "&token=" + Config.getToken();
