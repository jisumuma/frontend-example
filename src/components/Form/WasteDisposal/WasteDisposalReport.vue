				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Hazardous Waste Disposal Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="WasteDisposalReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		...
		methods: {
			draw: function() {
				$("#WasteDisposalReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/wasteDisposal/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#WasteDisposalReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/wasteDisposal/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Hazardous Waste Disposal');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#WasteDisposalReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/WasteDisposal/print?id=" + data.id + "&token=" + Config.getToken();
