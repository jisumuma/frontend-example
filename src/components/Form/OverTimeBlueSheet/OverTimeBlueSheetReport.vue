				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Blue Collar Planned Additional Working Time Sheet Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="OverTimeBlueSheetReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "OverTimeBlueSheetReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#OverTimeBlueSheetReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/overTimeBlue/exportrexcelsheet?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#OverTimeBlueSheetReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/overTimeBlue/reportsheet",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					$('td:eq(1)', row).html('System');
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Blue Collar Planned Additional Working Time Sheet');
					$('td:eq(4)', row).html('-');
			...
			$('#OverTimeBlueSheetReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/OverTimeBlueSheet/print?id=" + data.id + "&token=" + Config.getToken();
