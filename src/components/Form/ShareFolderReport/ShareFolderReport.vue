				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;ShareFolder Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="ShareFolderReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "ShareFolderReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#ShareFolderReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/ShareFolder/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#ShareFolderReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/ShareFolder/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
          ...
          if(data.status == 0) {
			...
			$('#ShareFolderReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/ShareFolder/print?id=" + data.id + "&token=" + Config.getToken();
