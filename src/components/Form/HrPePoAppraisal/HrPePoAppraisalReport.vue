				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Permanent Position Appraisal Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<form>
								<div class="text-muted well well-sm no-shadow">
									...
									<div class="row">
										<div class="form-group col-md-6" style="padding-top: 24px;margin-bottom: 0;">
											<button type="button" class="btn btn-primary" @click="onemonthemail();" title="One Month Email" style="margin-left: 5px;"><i class="fa fa-fw fa-external-link"></i> One Month Email</button>
											<button type="button" class="btn btn-primary mar-left-5" @click="twoweekemail();" title="Two Week Email" style="margin-left: 5px;"><i class="fa fa-fw fa-external-link"></i> Two Week Email</button>

										</div>
									</div>
								</div>
							</form>
							<table id="HrPePoAppraisalReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "HrPePoAppraisalReport",
		props: ['vclass'],
		methods: {
			onemonthemail:function(){
				var uri = "/HrPePoAppraisal/sendonemonthemail" ;
				global_.actUtil.query("get", uri, "", function(data) {
							var dm = JSON.parse(data);
							$.alert(dm.msg);
				});
			},
			twoweekemail:function(){
				var uri = "/HrPePoAppraisal/sendtwoweekemail" ;
				global_.actUtil.query("get", uri, "", function(data) {
							var dm = JSON.parse(data);
							$.alert(dm.msg);
				});
			...
			draw: function() {
				$("#HrPePoAppraisalReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/HrPePoAppraisal/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#HrPePoAppraisalReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/HrPePoAppraisal/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
          ...
          function formateDate(timestamp) { //全局方法 Vue.filter() 注册一个自定义过滤器,必须放在Vue实例化前面
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Permanent Position Appraisal');
					$('td:eq(4)', row).html('-');
          ...
          $('td:eq(9)', row).html('<a class="exportpdf"><i class="fa fa-file-pdf-o"></i></a>');
			...
			$('#HrPePoAppraisalReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/HrPePoAppraisal/print?id=" + data.id + "&token=" + Config.getToken();
