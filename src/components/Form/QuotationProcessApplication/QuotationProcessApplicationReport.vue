<template>
	<div>
		...
		<section class="content-header">
			<h1>Planning and Program Management Report</h1>
		</section>
				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Quotation Request Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="QuotationProcessApplicationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "QuotationProcessApplicationReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#QuotationProcessApplicationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/QuotationProcessApplication/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#QuotationProcessApplicationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/QuotationProcessApplication/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					if(data.createUserId != null && data.createUserId != data.applicantUserId){
						$('td:eq(2)', row).html('by '+data.createUserName);
					}else{
						$('td:eq(2)', row).html('-');
					}
					$('td:eq(3)', row).html('Quotation Request');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#QuotationProcessApplicationReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/QuotationProcessApplication/print?id=" + data.id + "&token=" + Config.getToken();
