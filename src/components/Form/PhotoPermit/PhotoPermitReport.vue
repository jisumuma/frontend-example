<template>
	<div>
		...
		<section class="content-header">
			<h1>
    			 Photo Permit Application Report
      </h1>
		</section>
				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Photo Permit Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="PhotoPermitReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "PhotoPermitReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#PhotoPermitReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/photoPermit/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#PhotoPermitReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/photoPermit/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Photo Permit Application');
					$('td:eq(4)', row).html('-');
		...
		$("body").keydown(function(event) {
			...
			$('#PhotoPermitReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/PhotoPermit/print?id=" + data.id + "&token=" + Config.getToken();
