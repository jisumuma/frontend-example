				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;E-Closing Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="EclosingApplicationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "EclosingApplicationReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#EclosingApplicationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/eclosingApplication/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#EclosingApplicationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/eclosingApplication/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					if(data.processKey=='EclosingApplicationNew'){
						$('td:eq(3)', row).html('E-Closing (HRECL)');
					}else{
						$('td:eq(3)', row).html('E-Closing');
					}
					...
					if(display != null && display.length > 40){
						var display2=display.substring(0, 40)+"...";
						display='<a href="###" title="'+display+'"><font color="#333">'+display2+'</font></a>';
					}
					if(data.status == 0) {
			...
			$('#EclosingApplicationReport tbody').on('click', '.exportpdf', function() {
				...
				var url="";
				console.log("data.processKey="+data.processKey);
				if(data.processKey=='EclosingApplicationNew'){
					url = Config.getBaseURL() + "/eclosingApplicationNew/print/" + data.id+ "?token=" + Config.getToken();
				}else{
					url = Config.getBaseURL() + "/eclosingApplication/print/" + data.id+ "?token=" + Config.getToken();
				}
				//var url = Config.getBaseURL() + "/eclosingApplication/print/" + data.id+ "?token=" + Config.getToken();
				//var url = Config.getBaseURL() + "/eclosingApplication/print?id=" + data.id + "&token=" + Config.getToken();
