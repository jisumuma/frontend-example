				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Bm  Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="BmApplicationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "BmApplicationReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#BmApplicationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/bmApplication/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#BmApplicationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/bmApplication/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('BM Application');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#BmApplicationReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/bmApplication/print?id=" + data.id + "&token=" + Config.getToken();
