				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Training Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="HrTrainingReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "HrTrainingReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#HrTrainingReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/HrTraining/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#HrTrainingReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/HrTraining/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					$('td:eq(3)', row).html('Training Application');
          ...
          if(data.status == 0) {
			...
			$('#HrTrainingReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/HrTraining/print?id=" + data.id + "&token=" + Config.getToken();
