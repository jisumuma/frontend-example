				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Work Certificate Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
								...
								<div class="text-muted well well-sm no-shadow">
									...
									<div class="row">
										...
										<div class="form-group col-md-3">
											...
											<input type="text" class="form-control" id="enName" placeholder="Applicant Name 姓名">
										</div>
										<!-- 搜索、导出按钮 -->
							...
							<table id="WorkCertificateReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "WorkCertificateReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#WorkCertificateReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/WorkCertificate/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#WorkCertificateReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/WorkCertificate/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Work Certificate');
					var certificateType="";
					if(data.certificateType=="Employment Certificate"){
						certificateType="Employment Certificate 工作证明";
					}else if(data.certificateType=="Income Certificate"){
						certificateType="Income Certificate 收入证明";
					}else if(data.certificateType=="Certificate for Visa"){
						certificateType="Certificate for Visa 签证证明";
					}else if(data.certificateType=="Special Template Certificate"){
						certificateType="Special Template Certificate 特殊模板证明";
					}
					$('td:eq(4)', row).html(certificateType);
          ...
          if(data.status == 0) {
			...
			$('#WorkCertificateReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/WorkCertificate/print?id=" + data.id + "&token=" + Config.getToken();
