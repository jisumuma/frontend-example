				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Team Building Application Report 团队建设报告</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="TeamBuildingReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "TeamBuildingReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#TeamBuildingReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/teambuilding/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#TeamBuildingReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/teambuilding/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Team Building Application');
					if(data.type=='1'){
						$('td:eq(4)', row).html('Team Building Workshop 团队建设（含团队研讨会）');
					}else if(data.type=='2'){
						$('td:eq(4)', row).html('Cross-Department Team Building 跨部门团建');
					}else{
						$('td:eq(4)', row).html('-');	
					}
          ...
          if(data.status == 0) {
			...
			$('#TeamBuildingReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/TeamBuilding/print?id=" + data.id + "&token=" + Config.getToken();
