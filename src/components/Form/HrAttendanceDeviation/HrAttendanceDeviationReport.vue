				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Attendance Deviation Application Report</h3>
						</div>
						<!-- /.box-header -->
                        ...
                        <option value="93">Expired</option>
							...
							<table id="HrAttendanceDeviationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "HrAttendanceDeviationReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#HrAttendanceDeviationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/hrAttendanceDeviation/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#HrAttendanceDeviationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/hrAttendanceDeviation/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Attendance Deviation Application');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#HrAttendanceDeviationReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/HrAttendanceDeviation/print?id=" + data.id + "&token=" + Config.getToken();
