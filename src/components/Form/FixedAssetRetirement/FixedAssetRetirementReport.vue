				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Fixed Asset Retirement Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="FixedAssetRetirementReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "FixedAssetRetirementReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#FixedAssetRetirementReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/FixedAssetRetirement/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#FixedAssetRetirementReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/FixedAssetRetirement/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Fixed Asset Retirement');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#FixedAssetRetirementReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/FixedAssetRetirement/print?id=" + data.id + "&token=" + Config.getToken();
