				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;WifiAccount Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="WifiAccountReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "WifiAccountReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#WifiAccountReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/WifiAccount/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#WifiAccountReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/WifiAccount/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('WifiAccount');
					$('td:eq(4)', row).html(data.applicationType==1?'New WIFI Account':'WIFI Account Extension');
          ...
          if(data.status == 0) {
			...
			$('#WifiAccountReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/WifiAccount/print?id=" + data.id + "&token=" + Config.getToken();
