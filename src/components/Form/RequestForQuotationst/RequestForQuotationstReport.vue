				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Request for Quotations Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<form>
								<div class="text-muted well well-sm no-shadow">
									<div class="row">
										...
										<div class="form-group col-md-3">
											<label for="">Application Email 申请邮箱</label>
											<input type="text" class="form-control" id="applicationEmail" placeholder="Application Email 申请邮箱">
										</div>
									</div>
									<div class="row">
										<!-- 申请单号 -->
							...
							<table id="RequestForQuotationstReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "RequestForQuotationstReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#RequestForQuotationstReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/requestForQuotationst/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#RequestForQuotationstReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/requestForQuotationst/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Request for Quotations');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
            ...
            statusHtml = '<span class="text-muted"><i class="fa fa-fw fa-file-text"></i> Draft</span>';
					...
					$('td:eq(8)', row).html('<a class="exportpdf"><i class="fa fa-file-pdf-o"></i></a> <a class="detailbj" href="javascript:;" style="margin-left: 10px;"> <i class="fa fa-file-text-o detail"></i></a>');
			...
			$('#RequestForQuotationstReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/RequestForQuotationst/print?id=" + data.id + "&token=" + Config.getToken();
			...
			$('#RequestForQuotationstReport tbody').on('click', '.detailbj', function () {
				var row = $(this).parents("tr");
				var data = table.row(row).data();
				router.push({ path: '/index/form/RequestForQuotationst/detail/'+data['id']+'?processInstanceId='+data['processInstanceId']});
			});
			//this.draw();
