				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Training Cancel Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="HrTrainingCancelReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "HrTrainingCancelReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#HrTrainingCancelReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/HrTrainingCancel/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#HrTrainingCancelReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/HrTrainingCancel/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					function typeString(type) {
						var typeName = "";
						if(type == "1") {
							typeName = "Interdisciplinary Training 通用技能培训";
						} else if(type == "2") {
							typeName = "Language Training 语言培训";
						}  else if(type == "3") {
							typeName = "Technical Training 专业技能培训";
						}  else if(type == "4") {
							typeName = "Personnel Development 人员发展培训";
						}
						return typeName;
					}
					$('td:eq(0)', row).html(data.ticketNo);
					$('td:eq(1)', row).html(data.creatorName);
					if(data.createUserName!=data.creatorName){
						$('td:eq(2)', row).html("By "+data.createUserName);
					}else{
						$('td:eq(2)', row).html('-');
					}
					$('td:eq(3)', row).html('Training Cancel');
					$('td:eq(4)', row).html(typeString(data.trainingType));
          ...
          if(data.status == 0) {
			...
			$('#HrTrainingCancelReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/HrTrainingCancel/print?id=" + data.id + "&token=" + Config.getToken();
