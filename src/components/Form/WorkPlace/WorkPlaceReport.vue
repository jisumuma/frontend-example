				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Workplace Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="WpApplicationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "WorkPlaceReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#WpApplicationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/workPlace/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#WpApplicationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/workPlace/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Workplace Application');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#WpApplicationReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/WorkPlace/print?id=" + data.id + "&token=" + Config.getToken();
