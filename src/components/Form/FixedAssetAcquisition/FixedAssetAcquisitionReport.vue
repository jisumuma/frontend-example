				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Fixed Asset Acquisition Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="FixedAssetAcquisitionReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "FixedAssetAcquisitionReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#FixedAssetAcquisitionReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/FixedAssetAcquisition/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#FixedAssetAcquisitionReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/FixedAssetAcquisition/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html(data.uploader);
					$('td:eq(3)', row).html('-');
					$('td:eq(4)', row).html('Fixed Asset Acquisition');
					$('td:eq(5)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#FixedAssetAcquisitionReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/FixedAssetAcquisition/print?id=" + data.id + "&token=" + Config.getToken();
