				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Company Phone Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="CompanyPhoneReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "ItcpCompanyPhoneReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#CompanyPhoneReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/itcpcompanyphone/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#CompanyPhoneReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/itcpcompanyphone/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Company Phone');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#CompanyPhoneReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/ItcpCompanyPhone/print?id=" + data.id + "&token=" + Config.getToken();
