  ...
  <div>
	<!-- Content Header (Page header) -->
	  <section class="content-header">
		<h1>Transportation Service Application Report</h1>
	  </section>
		<!-- Main content -->
				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Transportation Service Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="TransportationApplicationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "TransportationApplicationReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#TransportationApplicationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/transportationApplication/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#TransportationApplicationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/transportationApplication/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
				...
				"fnRowCallback": function(row, data, index) { //设置列从0开始
					...
					var sub = data.transType == 'D' ? 'Domestic 国内' : (data.transType == 'I' ? 'International 国际' : '');
					...
					$('td:eq(3)', row).html('Transportation Service');
			...
			$('#TransportationApplicationReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/TransportationApplication/print?id=" + data.id + "&token=" + Config.getToken();
