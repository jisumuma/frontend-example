				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Company Car Application Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="CompanyCarReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "CompanyCarReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#CompanyCarReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/companyCar/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#CompanyCarReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/companyCar/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Company Car Application');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#CompanyCarReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/CompanyCar/print?id=" + data.id + "&token=" + Config.getToken();
