				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Contract Extension Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="HrceReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "ContractExtensionReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#HrceReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/ContractExtension/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#HrceReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/ContractExtension/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Contract Extension');
					$('td:eq(4)', row).html('');
          ...
          if(data.status == 0) {
			...
			$('#HrceReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/ContractExtension/print?id=" + data.id + "&token=" + Config.getToken();
