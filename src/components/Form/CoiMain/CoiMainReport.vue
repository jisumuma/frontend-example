				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Declaration of Conflicts of Interest Report</h3>
						</div>
						<!-- /.box-header -->
                    ...
                    <div class="form-group col-md-3" style="margin-bottom: 0;">
                    <label for="">Applicant Name 申请员工姓名</label>
                      <input type="text" class="form-control" id="name" placeholder="Applicant Name 申请员工姓名">
                  </div>
									</div>
									<div class="row">
										<!-- 申请单号 -->
							...
							<table id="CoiMainReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
	...
	export default {
		...
		methods: {
			...
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/CoiMain/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#CoiMainReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/CoiMain/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Declaration of Conflicts of Interest');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#CoiMainReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/CoiMain/print?id=" + data.id + "&token=" + Config.getToken();
