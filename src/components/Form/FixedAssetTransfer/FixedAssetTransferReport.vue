				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Fixed Asset Transfer Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="FixedAssetTransferReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "FixedAssetTransferReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#FixedAssetTransferReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/FixedAssetTransfer/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#FixedAssetTransferReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/FixedAssetTransfer/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(3)', row).html('Fixed Asset Transfer');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#FixedAssetTransferReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/FixedAssetTransfer/print?id=" + data.id + "&token=" + Config.getToken();
