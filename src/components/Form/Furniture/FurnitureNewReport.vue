				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Furniture Service Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="FurnitureNewReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "FurnitureNewReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#FurnitureNewReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/FurnitureNew/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#FurnitureNewReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/FurnitureNew/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
          ...
          if(data.status == 0) {
			...
			$('#FurnitureNewReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/FurnitureNew/print?id=" + data.id + "&token=" + Config.getToken();
