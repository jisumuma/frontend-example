				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp; Company Car Repair | Maintenance | Accident Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="CarMaintainAccidentReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "CarMaintainAccidentReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#CarMaintainAccidentReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/CarMaintainAndAccident/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#CarMaintainAccidentReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/CarMaintainAndAccident/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Company Car Maintenance & Accident');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#CarMaintainAccidentReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/CarMaintainAndAccident/print?id=" + data.id + "&token=" + Config.getToken();
