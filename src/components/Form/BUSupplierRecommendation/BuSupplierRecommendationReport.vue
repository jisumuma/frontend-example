				...
				<div class="col-md-12">
					...
					<div class="box box-primary">
						<div class="box-header with-border">
							<h3 class="box-title"><i class="fa fa-file-text-o"></i> &nbsp;Business Unit´s Supplier Nomination Recommendation Report</h3>
						</div>
						<!-- /.box-header -->
						...
						<div class="box-body">
							...
							<table id="BuSupplierRecommendationReport" class="table table-bordered table-striped dataTable" role="grid" aria-describedby="example2_info">
								<thead>
									<tr>
...
<script>
	...
	export default {
		name: "BuSupplierRecommendationReport",
		props: ['vclass'],
		methods: {
			draw: function() {
				$("#BuSupplierRecommendationReport").DataTable().draw();
			},
			exportexcel: function() {
				...
				var url = Config.getBaseURL() + "/buSupplierRecommendation/exportrexcel?applicationNo=" + applicationNo +
		...
		mounted() {
			...
			var table = $("#BuSupplierRecommendationReport").DataTable({
				"fnServerData": queryData,
				"sAjaxSource": Config.getBaseURL() + "/buSupplierRecommendation/report",
				"serverSide": true,
				"filter": false,
				"lengthChange": false,
				"aoColumnDefs": [{
            ...
            return Y+M+D+h+m+s;
					...
					$('td:eq(2)', row).html('-');
					$('td:eq(3)', row).html('Business Unit´s Supplier Nomination Recommendation');
					$('td:eq(4)', row).html('-');
          ...
          if(data.status == 0) {
			...
			$('#BuSupplierRecommendationReport tbody').on('click', '.exportpdf', function() {
				...
				var url = Config.getBaseURL() + "/form/BuSupplierRecommendation/print?id=" + data.id + "&token=" + Config.getToken();
			...
			$('#BuSupplierRecommendationReport tbody').on('click', '.detailbj', function () {
				var row = $(this).parents("tr");
				var data = table.row(row).data();
				router.push({ path: '/index/form/BuSupplierRecommendation/detail/'+data['id']+'?processInstanceId='+data['processInstanceId']});
			});
			//this.draw();
