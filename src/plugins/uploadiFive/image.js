 ...
 * @returns {*}
 */
UPIMAGE.apply = function(o, c, defaults) {
    // 如果有默认属性
    if (defaults) {
        UPIMAGE.apply(o, defaults);
    }
    //将c中的属性值循环设定给o中
...
UPIMAGE.File = (function() {
    ...
    var defaultConfig = {
        ...
        onUploadComplete: function(file, data) {
            var me = this;
            UPIMAGE.File.onUploadComplete(file, data, me);
        },
    ...
    return {
        ...
        onUploadComplete: function(file, data, obj) {
            ...
            if (data) {
               ...
               var htmlString= "<li  id='file-id-"+data[0].id+"' data-attachment-id='"+data[0].id+"' x-file-id='"+id+"'>"+
				"<div><img  src='"+_BASE_URL+"/attachment/downloadimage/"+data[0].id+"' alt='"+data[0].attachName+"'></div>"+
				"<a class='users-list-name' target='_blank'  href='" + _BASE_URL + "/attachment/download/" + data[0].id + "'><i class='fa fa-download'></i>&nbsp;Download 下载</a>"+
				"<span class='users-list-date fileNameForImage'><a>"+data[0].attachName+"</a></span>"+
        ...
        init: function(id) {
            ...
            var $el = $("#" + id);
            var objConifg = {};
            UPIMAGE.apply(objConifg, defaultConfig);
            //读取个性化设置
