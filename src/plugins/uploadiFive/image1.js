 ...
 * @returns {*}
 */
UPIMAGE1.apply = function(o, c, defaults) {
    // 如果有默认属性
    if (defaults) {
        UPIMAGE1.apply(o, defaults);
    }
    //将c中的属性值循环设定给o中
...
UPIMAGE1.File = (function() {
    ...
    var defaultConfig = {
        ...
        onUploadComplete: function(file, data) {
            var me = this;
            UPIMAGE1.File.onUploadComplete(file, data, me);
        },
    ...
    return {
        ...
        onUploadComplete: function(file, data, obj) {
            ...
            if (data) {
				...
				"<div><img style='height: 80px;width: 100px;'  src='"+_BASE_URL+"/attachment/downloadimage/"+data[0].id+"' alt='"+data[0].attachName+"'></div>"+
				"<a style='display:none' class='users-list-name' target='_blank'  href='" + _BASE_URL + "/attachment/download/" + data[0].id + "'><i class='fa fa-download'></i>&nbsp;Download 下载</a>"+
        ...
        init: function(id) {
            var me = UPIMAGE1.File;
            //初始化文件上传
            var $el = $("#" + id);
            var objConifg = {};
            UPIMAGE1.apply(objConifg, defaultConfig);
            //读取个性化设置
