        ...
        init: function(c) {
            return this.each(function() {
                ...
                if (isNaN(f.fileSizeLimit)) {
                    ...
                    var e = parseInt(f.fileSizeLimit);
                   
                    if (f.fileSizeLimit.indexOf("KB") > -1) {
                        f.fileSizeLimit = e * 1024;
                    } else {
                        if (f.fileSizeLimit.indexOf("MB") > -1) {
                            f.fileSizeLimit = e * 1024*1024;
                           
                        } else {
                            if (f.fileSizeLimit.indexOf("GB") > -1) {
                                f.fileSizeLimit = e * 1024*1024*1024;
                            }
                        }
                    }
                } else {
                    ...
                    function() {
                        ...
                        if ((d.queue.count + l) > f.queueSizeLimit && f.queueSizeLimit !== 0) {
                            if (b.inArray("onError", f.overrideEvents) < 0) {
                                alert("选择的文件超过了最大值(" + f.queueSizeLimit + ").  请减少一些文件.");
                            }
                ...
                d.fileExistsInQueue = function(k) {
                    for (var j in d.inputs) {
                        var input = d.inputs[j];
                        var limit = input.files.length;
                        for (var l = 0; l < limit; l++) {
                            var existingFile = input.files[l];
                ...
                d.uploadFile = function(k, l) {
                    if (!k.skip && !k.complete && !k.uploading) {
                        ...
                        d.uploads.current++;
                        d.uploads.attempted++;
                        var xhr = k.xhr = new XMLHttpRequest();
                        if (typeof FormData === "function" || typeof FormData === "object") {
                            var m = new FormData();
                            m.append(f.fileObjName, k);
                            for (var i in f.formData) {
                                ...
                                if (this.readyState == 4) {
                                    ...
                                    if (this.status == 200) {
                                        if (k.xhr.responseText !== "Invalid file type.") {
                                            ...
                                            d.error("FORBIDDEN_FILE_TYPE", k, l);
                                        }
                                    } else {
                                        if (this.status == 404) {
                        ...
                        if (e.uploads.current == 0) {
                            if (b.inArray("onError", f.overrideEvents) < 0) {
                                if (e.filesToUpload() > 0 && f.uploadLimit != 0) {
                                    alert("The maximum upload limit has been reached.上传文件个数已达上限。");
                                }
                            }
                            if (typeof f.onError === "function") {
